...
def Writelog(self):
	Log = open(f'{self.ID}_Assembly_Assessment.txt','w')
	Log.write(f'ID:{self.ID}\n'+\
			  f'N50:{self.N50}\n'+\
			  f'NCount:{self.NCount}\n'+\
			  f'GapCount:{self.GapCount}\n'+\
			  f'AssemblySize:{self.AssemblySize}\n')
	Log.close()
...
class Assembly:
	def __init__(self,ID,fastapath,log=False):
		self.ID = ID
